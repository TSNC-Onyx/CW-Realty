...
const SEPARATOR = "[ .-]{0,3}";
const REQUIRED_SEPARATOR = "[ .-]{1,3}";
// 123-45-6789, 123.45.6789, 123 - 45 - 6789, or 123456789. Separated forms must break after
// the 3rd and 5th digits, so a ZIP+4 such as 27410-1234 is not mistaken for one.
const SSN_PATTERN = new RegExp(`\\b(?:\\d{3}${REQUIRED_SEPARATOR}\\d{2}${REQUIRED_SEPARATOR}\\d{4}|\\d{9})\\b`, "g");
...
// Shorter numbers (6 to 17 digits) right after words that name an account or ID. A bare "id"
// is left out: visitors ask about listing ids and MLS numbers.
const LABELED_NUMBER_PATTERN = new RegExp(
  `(?<=\\b(?:account|acct|routing|bank|card|ssn|social security|social|license|licence|passport|id number|id no|id #|tax id|ein|itin)\\b[^0-9]{0,30})\\d(?:${SEPARATOR}\\d){5,16}\\b`,
  "gi",
);
