...
async function submitRequestForm(form: FormDefinition, formData: FormData): Promise<RequestFormState> {
  ...
  try {
    await submitNewRequest({ ...request, idempotencyKey: String(formData.get("idempotencyKey") ?? crypto.randomUUID()) });
  } catch (error) {
    ...
    return getResultState({ status: "failed", values });
  }
  return getResultState({ status: "sent", values: {}, sentTo: { name: request.contactName, email: request.contactEmail } });
}
