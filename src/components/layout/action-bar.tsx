...
// Chat opens the Contact page until the chat assistant ships (Phase 5).

type ActionItem = { label: string; href: string; icon: LucideIcon };

function getActionItems(contact: ContactLinks | null): ActionItem[] {
  return [
    ...
    { label: "Chat", href: CONTACT_PAGE_PATH, icon: MessageCircle },
  ];
}
  ...
  return (
    <nav aria-label="Quick contact" className="tone-dark action-bar-safe-area fixed inset-x-0 bottom-0 z-30 border-t border-divider-dark lg:hidden">
      <ul className="grid grid-cols-3">
        {getActionItems(contact).map(({ label, href, icon: Icon }) => (
          <li key={label}>
            <a href={href} className="flex h-15 flex-col items-center justify-center gap-1 text-tag leading-none font-bold">
